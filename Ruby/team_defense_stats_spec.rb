...
describe TeamDefenseStats do
  describe '#build_insert' do
    it 'should return a well formed SQL insert statement' do
      ...
      expect( team_defense_stats.build_insert ).to eq 'INSERT INTO Team_Defense_Stats_T ( Team_Id, Season, Bowl_Game, Sacks, Interceptions, Return_Yards, Return_Touchdowns ) VALUES ( ?, ?, ?, ?, ?, ?, ? )'
    end
  end
  ...
  describe '#build_select' do
    it 'should return a well formed SQL select statement' do
      team_defense_stats = TeamDefenseStats.new
      expect( team_defense_stats.build_select ).to eq 'SELECT Team_Id, Season, Bowl_Game, Sacks, Interceptions, Return_Yards, Return_Touchdowns FROM Team_Defense_Stats_T WHERE Team_Id = ? AND Season = ? AND Bowl_Game = ?'
    end
  end
  ...
  describe '#build_update' do
    it 'should return a well formed SQL update statement' do
      team_defense_stats = TeamDefenseStats.new
      expect( team_defense_stats.build_update ).to eq 'UPDATE Team_Defense_Stats_T SET Sacks = ?, Interceptions = ?, Return_Yards = ?, Return_Touchdowns = ? WHERE Team_Id = ? AND Season = ? AND Bowl_Game = ?'
    end
  end
  ...
  describe '#build_delete' do
    it 'should return a well formed SQL update statement' do
      team_defense_stats = TeamDefenseStats.new
      expect( team_defense_stats.build_delete ).to eq 'DELETE FROM Team_Defense_Stats_T WHERE Team_Id = ? AND Season = ? AND Bowl_Game = ?'
    end
  end
