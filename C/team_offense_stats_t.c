...
static int team_offense_stats_t_create_bindings( sqlite3_stmt *statement, const void *data )
{
     ...
     BIND_INT( statement,  6, team_offense_stats->interceptions   );
     BIND_INT( statement,  7, team_offense_stats->pass_yards      );
     BIND_INT( statement,  8, team_offense_stats->pass_touchdowns );
     BIND_INT( statement,  9, team_offense_stats->rush_attempts   );
     BIND_INT( statement, 10, team_offense_stats->rush_yards      );
     BIND_INT( statement, 11, team_offense_stats->rush_touchdowns );
...
{
     static char query[]   = "INSERT INTO Team_Offense_Stats_T ( Team_Id, Season, Bowl_Game, Pass_Attempts, Completions, Interceptions, Pass_Yards, Pass_Touchdowns, Rush_Attempts, Rush_Yards, Rush_Touchdowns )"
          /**/                                         "VALUES ( ?,"     "?,"    "?,"       "?,"           "?,"         "?,"           "?,"        "?,"             "?,"           "?,"        "?"             ")";
...
static int team_offense_stats_t_read_retrieve( sqlite3_stmt *statement, const void *data )
{
     ...
     team_offense_stats->interceptions   = sqlite3_column_int( statement,  2 );
     team_offense_stats->pass_yards      = sqlite3_column_int( statement,  3 );
     team_offense_stats->pass_touchdowns = sqlite3_column_int( statement,  4 );
     team_offense_stats->rush_attempts   = sqlite3_column_int( statement,  5 );
     team_offense_stats->rush_yards      = sqlite3_column_int( statement,  6 );
     team_offense_stats->rush_touchdowns = sqlite3_column_int( statement,  7 );
...
static int team_offense_stats_t_read_retrieve_by_team( sqlite3_stmt *statement, const void *data )
{
     ...
     team_offense_stats.interceptions   = sqlite3_column_int( statement,  5 );
     team_offense_stats.pass_yards      = sqlite3_column_int( statement,  6 );
     team_offense_stats.pass_touchdowns = sqlite3_column_int( statement,  7 );
     team_offense_stats.rush_attempts   = sqlite3_column_int( statement,  8 );
     team_offense_stats.rush_yards      = sqlite3_column_int( statement,  9 );
     team_offense_stats.rush_touchdowns = sqlite3_column_int( statement, 10 );
...
{
     static char query[] = "SELECT Team_Id, Season, Bowl_Game, Pass_Attempts, Completions, Interceptions, Pass_Yards, Pass_Touchdowns, Rush_Attempts, Rush_Yards, Rush_Touchdowns FROM Team_Offense_Stats_T WHERE Team_Id = ?";
...
static int team_offense_stats_t_update_bindings( sqlite3_stmt *statement, const void *data )
{
     ...
     BIND_INT( statement,  3, team_offense_stats->interceptions   );
     BIND_INT( statement,  4, team_offense_stats->pass_yards      );
     BIND_INT( statement,  5, team_offense_stats->pass_touchdowns );
     BIND_INT( statement,  6, team_offense_stats->rush_attempts   );
     BIND_INT( statement,  7, team_offense_stats->rush_yards      );
     BIND_INT( statement,  8, team_offense_stats->rush_touchdowns );

     BIND_INT( statement,  9, team_offense_stats->team_id   );
     BIND_INT( statement, 10, team_offense_stats->season    );
     BIND_INT( statement, 11, team_offense_stats->bowl_game );
